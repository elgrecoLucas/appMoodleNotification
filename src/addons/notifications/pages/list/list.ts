...
export class AddonNotificationsListPage implements OnInit, OnDestroy {
    ...
    async deleteNotification(notification: AddonNotificationsNotificationToRender): Promise<void>{
        try {
            await AddonNotifications.deleteNotification(notification.id);
            CoreDomUtils.showToast('addon.notifications.successfullydeletedmessage', true, undefined);
            this.refreshNotifications();
        } catch (error) {
            CoreDomUtils.showErrorModalDefault(error, 'addon.notifications.errordeletingmessage', true);
        }
    }

    /**
     * Convenience function to get notifications. Gets unread notifications first.
     * Recibe notificaciones no leídas primero. 
     ...
     * Marque todas las notificaciones como leídas.
    ...
    protected async markNotificationsAsRead(notifications: AddonNotificationsNotificationToRender[]): Promise<void> {
        ...
        //Compruebe si se debe mostrar marcar todo como leído (hay notificaciones no leídas). 
        try {
            this.loadingMarkAllNotificationsAsRead = true;
